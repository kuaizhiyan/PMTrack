...
class PAMSTransformerDecoder(MyConditionalDetrTransformerDecoder):
...
class PAMSTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        super(PAMSTransformer,self).__init__()
        # process args
    ...
    def _init_layers(self) -> None:
        ...
        self.decoder = PAMSTransformerDecoder(**self.decoder)
...
class PMNet(BaseModule):
    ...
    Returns:
        - hidden_state (tuple): tuple of output hidden state (global query) of each layer.
    """
    ...
    def forward(self,x):    
        ...
        out = self.pyramid_sampler(x)  
        out,pos_attn_score_layers = self.mspctrans(out)
        # fetch out the first query 
